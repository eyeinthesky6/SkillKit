...
const deceptivePatterns = [
  ...
  // AI Parroting Patterns (from SEDI "Building with AI" docs)
  // Parroting & Claim Repetition
  { regex: /\b(parroting|parroted|parrots|repeating|repeated|repeats)\s+(claim|claims|statement|statements|assertion|assertions)\s+(without|without.*verification|without.*evidence|lacking|missing)\s+(verification|evidence|proof|independent.*validation|new.*evidence)\b/gi, type: "AI_PARROTING", severity: "CRITICAL", category: "deceptive" },
  { regex: /\b(same|identical|repeated)\s+(claim|statement|assertion|report)\s+(appears|appeared|appearing)\s+(across|in|over)\s+(multiple|several|many)\s+(sessions|files|documents|logs)\b/gi, type: "AI_PARROTING", severity: "CRITICAL", category: "deceptive" },
  { regex: /\b(cross.*session|cross.*file|cross.*document)\s+(contamination|amplification|reinforcement|repetition)\b/gi, type: "AI_PARROTING", severity: "HIGH", category: "deceptive" },
  
  // Sentiment Mirroring & Echo Chamber
  { regex: /\b(mirror|mirrored|mirroring|echo|echoed|echoing)\s+(user|sentiment|emotional.*state|optimism|pessimism)\s+(without|without.*validation|without.*verification|lacking|missing)\s+(validation|verification|technical.*validation|independent.*check)\b/gi, type: "SENTIMENT_MIRRORING", severity: "CRITICAL", category: "deceptive" },
  { regex: /\b(user|sentiment|emotional)\s+(echo.*chamber|amplification|mirroring|reinforcement)\b/gi, type: "SENTIMENT_MIRRORING", severity: "HIGH", category: "deceptive" },
  { regex: /\b(detects|detected|detecting)\s+(user|positive|optimistic)\s+(sentiment|emotional.*state).*\b(amplifies|amplified|amplifying|mirrors|mirrored|mirroring)\s+(without|without.*validation|lacking|missing)\s+(validation|verification|technical.*check)\b/gi, type: "SENTIMENT_MIRRORING", severity: "CRITICAL", category: "deceptive" },
  
  // Performance Theater & Cosmetic Compliance
  { regex: /\b(impressive.*looking|convincing.*looking|professional.*sounding|elaborate.*looking)\s+(report|analysis|documentation|summary|audit|document)\s+(without|lacking|missing)\s+(substance|verification|evidence|proof|actual.*work|thorough.*analysis)\b/gi, type: "PERFORMANCE_THEATER", severity: "CRITICAL", category: "deceptive" },
  { regex: /\b(cosmetic|surface.*level|superficial)\s+(compliance|fix|fixes|solution|solutions|changes)\s+(that|which)\s+(look|looks|looked|appear|appears|appeared)\s+(good|impressive|professional|complete)\s+(but|however|yet)\s+(aren.*t|are.*not|is.*not|don.*t|doesn.*t)\s+(real|actual|genuine|substantive|thorough)\b/gi, type: "COSMETIC_COMPLIANCE", severity: "CRITICAL", category: "deceptive" },
  { regex: /\b(performance|performing|performed)\s+(theater|theatre|show|display)\s+(instead.*of|rather.*than|without)\s+(actual|real|genuine|thorough)\s+(work|analysis|verification|checking)\b/gi, type: "PERFORMANCE_THEATER", severity: "CRITICAL", category: "deceptive" },
  
  // Rule Shopping & Creative Interpretation
  { regex: /\b(rule|rules)\s+(shopping|shopped|shop)\s+(finding|found|finds|picking|picked|picks)\s+(conflicting|most.*permissive|most.*lenient|conflicting)\s+(rules|rule|interpretation|interpretations)\b/gi, type: "RULE_SHOPPING", severity: "CRITICAL", category: "deceptive" },
  { regex: /\b(creative|creative.*interpretation|twisting|twisted|twists)\s+(rule|rules|meaning|meanings|interpretation|interpretations)\s+(to|in.*order.*to|so.*as.*to)\s+(justify|justified|justifies|rationalize|rationalized|rationalizes)\s+(desired|wanted|intended)\s+(action|actions|behavior|behaviors)\b/gi, type: "CREATIVE_INTERPRETATION", severity: "HIGH", category: "deceptive" },
  { regex: /\b(finds|found|finding|finds|picks|picked|picking)\s+(loophole|loopholes|workaround|workarounds|bypass|bypasses)\s+(in|within|through)\s+(rule|rules|system|systems|process|processes|protocol|protocols)\b/gi, type: "RULE_SHOPPING", severity: "HIGH", category: "deceptive" },
  
  // Progressive Claim Inflation
  { regex: /\b(progressive|progressively|escalating|escalated|escalates)\s+(claim|claims|assertion|assertions|optimism|optimistic.*claims)\s+(become|becomes|becoming|became|inflation|inflated|inflating)\s+(more|increasingly)\s+(optimistic|confident|ambitious|extreme|extreme)\b/gi, type: "CLAIM_INFLATION", severity: "CRITICAL", category: "deceptive" },
  { regex: /\b(claim|claims|status|report)\s+(becomes|became|becoming|escalates|escalated|escalating)\s+(more|increasingly)\s+(optimistic|confident|ambitious|extreme)\s+(over|across|throughout)\s+(time|sessions|interactions|iterations)\b/gi, type: "CLAIM_INFLATION", severity: "HIGH", category: "deceptive" },
  
  // Confirmation Bias & Status Quo Preservation
  { regex: /\b(confirmation|confirming|confirms|confirmed)\s+(bias|seeking|seeks|sought)\s+(evidence|proof|data|information)\s+(that|which)\s+(confirms|supports|validates|reinforces)\s+(previous|prior|existing|assumed)\s+(claim|claims|assumption|assumptions|belief|beliefs)\b/gi, type: "CONFIRMATION_BIAS", severity: "HIGH", category: "deceptive" },
  { regex: /\b(status.*quo|previous|prior|existing)\s+(preservation|preserving|preserves|maintenance|maintaining|maintains)\s+(reluctant|unwilling|hesitant)\s+(to|to.*contradict|to.*challenge|to.*correct)\s+(previous|prior|existing|earlier)\s+(work|claims|statements|AI|agent|agents)\b/gi, type: "STATUS_QUO_PRESERVATION", severity: "HIGH", category: "deceptive" },
  { regex: /\b(dismisses|dismissed|dismissing|ignores|ignored|ignoring|rationalizes|rationalized|rationalizing)\s+(contradictory|opposing|conflicting|contradicting)\s+(evidence|proof|data|information|facts)\b/gi, type: "CONFIRMATION_BIAS", severity: "CRITICAL", category: "deceptive" },
  
  // Optimistic Reporting & Bias
  { regex: /\b(optimistic|optimism)\s+(bias|reporting|reports|reported|claims|claiming)\s+(defaults|defaulting|defaulted)\s+(to|toward|towards)\s+(positive|optimistic|favorable)\s+(interpretation|interpretations|assessment|assessments|conclusion|conclusions)\b/gi, type: "OPTIMISM_BIAS", severity: "HIGH", category: "deceptive" },
  { regex: /\b(reports|reported|reporting|claims|claimed|claiming)\s+(success|completion|readiness|progress)\s+(based.*on|from|due.*to)\s+(intentions|plans|assumptions|expectations|hopes)\s+(rather.*than|instead.*of|not|without)\s+(reality|verification|evidence|proof|actual.*results|tested.*results)\b/gi, type: "OPTIMISTIC_REPORTING", severity: "CRITICAL", category: "deceptive" },
  { regex: /\b(partial|incomplete|partial.*success)\s+(success|completion|progress|implementation)\s+(reported|reports|reporting|claimed|claims|claiming)\s+(as|as.*if|like)\s+(complete|full|total|100%|entire)\s+(success|completion|progress|implementation)\b/gi, type: "OPTIMISTIC_REPORTING", severity: "HIGH", category: "deceptive" },
  
  // Task Execution Drift
  { regex: /\b(task|tasks)\s+(execution|executing|executed)\s+(drift|drifted|drifting|drifts)\s+(from|away.*from|off.*from)\s+(original|initial|intended|planned)\s+(task|scope|requirement|requirements|objective|objectives|goal|goals)\b/gi, type: "TASK_DRIFT", severity: "CRITICAL", category: "deceptive" },
  { regex: /\b(drifts|drifted|drifting|drift)\s+(from|away.*from|off.*from)\s+(original|initial|intended|planned)\s+(task|scope|requirement|requirements|objective|objectives|goal|goals)\s+(during|while|when|as)\s+(error|errors|fixing|fix|fixes|problem|problems|solving|solving)\s+(without|without.*maintaining|without.*returning.*to|lacking|missing)\s+(task|scope|context|original.*requirement|original.*goal)\s+(context|maintenance|return|validation)\b/gi, type: "TASK_DRIFT", severity: "CRITICAL", category: "deceptive" },
  { regex: /\b(follows|following|followed)\s+(error|errors|problem|problems|issue|issues)\s+(chain|chains|sequence|sequences|trail|trails)\s+(without|without.*maintaining|without.*returning.*to|lacking|missing)\s+(task|original|initial|intended|planned)\s+(context|scope|requirement|requirements|objective|objectives|goal|goals)\b/gi, type: "TASK_DRIFT", severity: "CRITICAL", category: "deceptive" },
  
  // Documentation Drift & Memory Contradiction
  { regex: /\b(documentation|document|documents|memory|memories)\s+(drift|drifted|drifting|drifts|contradiction|contradictions|contradicts|contradicted|contradicting)\s+(claim|claims|states|stated|says|said|saying)\s+(doesn.*t|does.*not|do.*not|don.*t|is.*not|are.*not|not)\s+(match|matches|matched|matching|align|aligns|aligned|aligning|reflect|reflects|reflected|reflecting)\s+(reality|codebase|actual|current|real|actual.*state|current.*state)\b/gi, type: "DOCUMENTATION_DRIFT", severity: "CRITICAL", category: "deceptive" },
  { regex: /\b(trusts|trusted|trusting|trust)\s+(memory|memories|stored.*information|previous.*claim|previous.*statement)\s+(over|instead.*of|rather.*than|despite)\s+(current|actual|real|present|existing|contradictory|opposing)\s+(evidence|proof|data|information|facts|reality|codebase|state)\b/gi, type: "MEMORY_CONTRADICTION", severity: "CRITICAL", category: "deceptive" },
  { regex: /\b(memory|memories|documentation|document|documents)\s+(becomes|became|becoming|becomes|treated|treats|treating)\s+(as|like)\s+(fact|facts|truth|truths|reality|realities)\s+(that|which)\s+(resists|resisted|resisting|resists|ignores|ignored|ignoring|ignores|dismisses|dismissed|dismissing|dismisses)\s+(contradiction|contradictions|contradictory|opposing|conflicting)\s+(evidence|proof|data|information|facts)\b/gi, type: "MEMORY_CONTRADICTION", severity: "CRITICAL", category: "deceptive" },
  
  // Bypass Innovation & Workaround Finding
  { regex: /\b(bypass|bypassed|bypassing|bypasses|circumvent|circumvented|circumventing|circumvents)\s+(innovation|innovations|finding|finds|found|discovering|discovers|discovered)\s+(technical|technical.*workaround|workaround|workarounds|loophole|loopholes|method|methods|way|ways)\s+(to|in.*order.*to|so.*as.*to)\s+(avoid|avoiding|avoids|evade|evading|evades|get.*around|getting.*around|gets.*around)\s+(restriction|restrictions|rule|rules|control|controls|constraint|constraints|limitation|limitations)\b/gi, type: "BYPASS_INNOVATION", severity: "CRITICAL", category: "deceptive" },
  { regex: /\b(finds|found|finding|finds|discovers|discovered|discovering|discovers)\s+(technical|creative|innovative|clever|sophisticated)\s+(workaround|workarounds|loophole|loopholes|method|methods|way|ways|solution|solutions)\s+(to|in.*order.*to|so.*as.*to)\s+(avoid|avoiding|avoids|evade|evading|evades|get.*around|getting.*around|gets.*around|bypass|bypassing|bypassed|bypasses)\s+(quality.*gate|quality.*control|verification|testing|review|check|audit|lint|eslint|typescript|build.*check|rule|rules|restriction|restrictions)\b/gi, type: "BYPASS_INNOVATION", severity: "CRITICAL", category: "deceptive" },
