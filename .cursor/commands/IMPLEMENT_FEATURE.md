...
## Phase 0: Check AITracking (REQUIRED)

**See:** @docs/workflows/subtasks/aitracking.md

**MANDATORY: Before starting, check for similar work:**

```bash
# Search for similar features:
grep -r "feature_name" docs/AITracking/ 2>/dev/null | head -10

# Check today's work:
ls -t docs/AITracking/AIAction_$(date +%d-%m-%Y)_*.md 2>/dev/null | head -3
```

**Create new AITracking log:**
```bash
TODAY=$(date +%d-%m-%Y)
TASK_NAME="implement_feature_name"
LOG_FILE="docs/AITracking/AIAction_${TODAY}_${TASK_NAME}.md"
# Create log file (see aitracking.md subtask for template)
```
...
3. **MANDATORY:** Finalize AITracking log:
   **See:** @docs/workflows/subtasks/aitracking.md
   ...
   echo "- Feature implemented successfully" >> "$LOG_FILE"
   echo "- Files created: [list]" >> "$LOG_FILE"
   ```
